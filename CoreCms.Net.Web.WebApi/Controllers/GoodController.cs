...
namespace CoreCms.Net.Web.WebApi.Controllers
{
    ...
    public class GoodController : ControllerBase
    {
        ...
        /// </summary>
        /// <returns></returns>
        [HttpPost]  // 标明这是一个 POST 请求的接口方法
        public async Task<WebApiCallBack> GetAllCategories()  // 异步方法，返回 WebApiCallBack 类型的数据
        {
            var jm = new WebApiCallBack() { status = true };  // 创建 WebApiCallBack 对象并初始化状态为 true

            // 调用 _goodsCategoryServices 的 QueryListByClauseAsync 方法来获取所有商品分类数据
            // 条件是分类显示（isShow == true），并按排序字段（sort）升序排列
            var data = await _goodsCategoryServices.QueryListByClauseAsync(p => p.isShow == true, p => p.sort,
                OrderByType.Asc);

            var wxGoodCategoryDto = new List<WxGoodCategoryDto>();  // 创建一个列表用于存储最终返回的数据

            // 找到所有父级分类（parentId == 0），并进行遍历
            var parents = data.Where(p => p.parentId == 0).ToList();
            if (parents.Any())  // 如果存在父级分类
            {
                parents.ForEach(p =>
                {
                    var model = new WxGoodCategoryDto();  // 创建一个新的 WxGoodCategoryDto 对象来存储分类信息
                    model.id = p.id;  // 设置分类 ID
                    model.name = p.name;  // 设置分类名称
                                          // 如果图片 URL 不为空，使用图片 URL，否则使用默认图片
                    model.imageUrl = !string.IsNullOrEmpty(p.imageUrl) ? p.imageUrl : "/static/images/common/empty.png";
                    model.sort = p.sort;  // 设置分类排序

                    // 查找当前父分类下的子分类（parentId == 当前父分类的 id）
                    var childs = data.Where(p => p.parentId == model.id).ToList();
                    if (childs.Any())  // 如果存在子分类
                    {
                        var childsList = new List<WxGoodCategoryChild>();  // 创建子分类的列表
                        childs.ForEach(o =>
                        {
                            childsList.Add(new WxGoodCategoryChild()  // 为每个子分类创建 WxGoodCategoryChild 对象
                            {
                                id = o.id,  // 子分类 ID
                                imageUrl = !string.IsNullOrEmpty(o.imageUrl) ? o.imageUrl : "/static/images/common/empty.png",  // 子分类图片 URL
                                name = o.name,  // 子分类名称
                                sort = o.sort  // 子分类排序
                            });
                        });
                        model.child = childsList;  // 将子分类列表赋值给父分类的子分类属性
                    }
                    wxGoodCategoryDto.Add(model);  // 将父分类添加到最终的列表中
                });
            }

            jm.status = true;  // 设置 WebApiCallBack 的状态为成功
            jm.data = wxGoodCategoryDto;  // 将分类数据（包含父分类和子分类）赋值给返回对象

            return jm;  // 返回 WebApiCallBack 对象，包含分类数据
        }
        #endregion
